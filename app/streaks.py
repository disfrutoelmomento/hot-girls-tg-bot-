...
def calendar_days(db: Session, user_id: int, days: int = 90, since: date | None = None) -> list[dict]:
    ...
    App (по типу GitHub contributions), заполняется слева направо: сентябрь,
    октябрь, ноябрь... по мере того как проходит время. Обычно это
    последние `days` дней, но если участница зарегистрировалась позже
    этого окна, начинаем прямо с даты регистрации — не тянуть же пустые
    месяцы до того, как бот вообще существовал."""
    dates = _checkin_dates(db, user_id)
    end = today_msk()

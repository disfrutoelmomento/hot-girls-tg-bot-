...
- Стикеры: перешли боту стикеры, которые нравятся — я заберу их file_id
  (так же, как получила chat_id группы) и впишу в BADGE_STICKERS /
  DAY_SUCCESS_STICKERS ниже.
...
# Стикеры для радостных моментов (badge / удачное закрытие дня). Пусто по
# умолчанию — см. инструкцию в начале файла.
BADGE_STICKERS: list[str] = []
DAY_SUCCESS_STICKERS: list[str] = []

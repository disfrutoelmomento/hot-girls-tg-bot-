...
- Стикеры: любая из участниц шлёт боту стикер в личку — он сам сохранится
  в общую копилку (БД). Один пул для всех: какие именно уведомления его
  используют — решается в коде (сейчас: бейдж, утреннее приветствие,
  удачное закрытие дня), не при отправке стикера. Ничего в этом файле
  трогать не нужно (см. app/handlers/sticker.py и app/notify.py).

...
class Sticker(Base):
    ...
    попадает в общую копилку. Какие именно уведомления подмешивают
    случайный стикер из этой копилки — решается в коде (app/notify.py,
    app/scheduler.py), а не при отправке стикера. UNIQUE на file_id
    защищает от дублей, если один и тот же стикер пришлют дважды (в том
    числе от разных участниц)."""
